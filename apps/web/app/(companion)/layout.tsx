...
export default function CompanionLayout({ children }: { children: React.ReactNode }) {
  ...
  const isBare =
    pathname === '/companion/legal' || pathname.startsWith('/companion/onboarding/legal')

  if (isBare) {
    return <div className="min-h-screen bg-[#07090f] relative overflow-x-hidden">{children}</div>
  }
