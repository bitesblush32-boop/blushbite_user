...
function Shimmer({ h, r = 12 }: { h: number; r?: number }) {
  return <div className="shimmer" style={{ height: h, borderRadius: r, background: '#111620' }} />
}
    ...
    <div style={{ position: 'relative' }}>
      ...
      >
        ...
        <span style={{ fontSize: 10, color: '#4b5563' }}>
          {fmtDate(padded[Math.floor(padded.length / 2)].date)}
        </span>
        <span style={{ fontSize: 10, color: '#4b5563' }}>
          {fmtDate(padded[padded.length - 1].date)}
        </span>
      </div>
    </div>
  )
...
export default function CompanionAnalyticsPage() {
  const { data, isLoading } = useQuery<AnalyticsSummary>({
    ...
    queryFn: () =>
      fetch('/api/companions/analytics/summary', { credentials: 'include' }).then((r) => r.json()),
    staleTime: 60_000,
  ...
  return (
    ...
    >
      ...
      {!isLoading && (
        ...
        >
          {visible ? <Wifi size={16} color="#34d399" /> : <WifiOff size={16} color="#e8607a" />}
          <div style={{ flex: 1 }}>
      ...
      >
        ...
        {isLoading ? <Shimmer h={106} r={8} /> : <BarChart data={data?.chart ?? []} />}
      </div>
    </motion.div>
  )
}
