...
import {
  Bell, Eye, Link2, CheckCircle2, Star, Calendar,
} from 'lucide-react'
...
type FilterKey = 'all' | 'profile_viewed' | 'story_linked' | 'bridge_approved' | 'booking_request' | 'admin_approved'
...
function notifIcon(type: string) {
  switch (type) {
    case 'profile_viewed':  return { Icon: Eye,          bg: 'rgba(232,96,122,0.12)',    color: '#e8607a' }
    case 'story_linked':    return { Icon: Link2,         bg: 'rgba(201,169,110,0.12)',   color: '#c9a96e' }
    case 'bridge_approved': return { Icon: CheckCircle2,  bg: 'rgba(52,211,153,0.12)',    color: '#34d399' }
    case 'admin_approved':  return { Icon: Star,          bg: 'rgba(201,169,110,0.12)',   color: '#c9a96e' }
    case 'booking_request': return { Icon: Calendar,      bg: 'rgba(201,169,110,0.12)',   color: '#c9a96e' }
    default:                return { Icon: Bell,          bg: 'rgba(107,114,128,0.12)',   color: '#6b7280' }
  }
}
...
export default function CompanionNotificationsPage() {
  ...
  const unreadIds = notifications.filter((n: NotificationItem) => !n.is_read).map((n: NotificationItem) => n.id)
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          {unreadCount > 0 && (
            <span style={{
              ...
              background: 'rgba(232,96,122,0.12)', border: '1px solid rgba(232,96,122,0.3)',
              color: '#e8607a',
            }}>
              {unreadCount} new
            </span>
        ...
        {unreadIds.length > 0 && (
          <button
            ...
            style={{
              fontSize: 12, color: '#e8607a', background: 'none', border: 'none',
      ...
      {isLoading ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {Array.from({ length: 5 }).map((_, i) => (
            <div key={i} className="shimmer" style={{ height: 76, borderRadius: 14, background: '#111620' }} />
          ))}
        </div>
      ) : filtered.length === 0 ? (
        <div style={{
          ...
          {filtered.map((n: NotificationItem) => {
            ...
            return (
              <motion.div
                ...
                onMouseEnter={e => { if (n.action_url) (e.currentTarget as HTMLDivElement).style.background = '#161d2a' }}
                onMouseLeave={e => { (e.currentTarget as HTMLDivElement).style.background = '#111620' }}
              >
                {/* Icon */}
                <div style={{
                  width:          36,
                ...
                {!n.is_read && (
                  <div style={{
                    ...
                    background: '#e8607a', flexShrink: 0, marginTop: 4,
                  }} />
                )}
              </motion.div>
            )
