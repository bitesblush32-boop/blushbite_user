...
function FieldWrap({
  ...
  return (
    <div className="mb-5">
      ...
      {children}
      {helper && (
        <p style={{ fontSize: 11, color: '#6b7280', fontStyle: 'italic', marginTop: 5 }}>
          {helper}
        </p>
      )}
    </div>
...
export default function EditProfilePage() {
  ...
  const [verifyStatus, setVerifyStatus] = useState<
    'idle' | 'running' | 'pending' | 'approved' | 'declined' | 'skipped'
  >('idle')
  ...
  const [sessionModality, setSessionModality] = useState<'in_person' | 'online' | 'both'>(
    'in_person'
  )
  ...
  // ── Photo upload ──
  const processPhotoFiles = useCallback(
    async (files: File[]) => {
  ...
  async function processVideoFile(file: File) {
    ...
    if (duration > 10) {
      setVideoError('Video must be 10 seconds or under.')
      return
    }
    if (file.size > 50 * 1024 * 1024) {
      setVideoError('Max file size is 50MB.')
      return
    }
    ...
    >
      ...
      >
        <SectionHeader
          ...
          subtitle="Legal details are locked after submission."
        />
      ...
      >
        <SectionHeader
          ...
          subtitle="This is what dreamers see when they discover you."
        />
      ...
      >
        <SectionHeader
          letter="C"
          title="Your Identity on BlushBite"
          subtitle="Help dreamers find you. Be honest — mismatched tags lead to wrong-fit bookings."
        />

        {/* Fantasy tags */}
        <div className="mb-8">
          <p style={{ fontSize: 13, color: '#eeeef0', marginBottom: 6, fontWeight: 500 }}>
            Fantasy interest tags
          </p>
          <p style={{ fontSize: 12, color: '#6b7280', marginBottom: 12 }}>
            Select 3–7 tags that genuinely match you.
          </p>
        ...
        <div className="mb-4">
          ...
          <p style={{ fontSize: 12, color: '#6b7280', marginBottom: 12 }}>
            Languages you can host sessions in.
          </p>
          <div className="flex flex-wrap gap-2">
            {LANGUAGES.map((lang) => {
      ...
      >
        <SectionHeader
          letter="D"
          title="Services & Rate Chart"
          subtitle="How dreamers can book you."
        />
          ...
          <div>
            {cards.map((card) => (
              ...
              >
                ...
                <div className="mb-4">
                  <label
                    style={{ fontSize: 11, color: '#6b7280', display: 'block', marginBottom: 5 }}
                  >
                    Service title
                  </label>
                  <input
                    ...
                    onChange={(e) =>
                      setCards((p) =>
                        p.map((c) => (c.id === card.id ? { ...c, title: e.target.value } : c))
                      )
                    }
                    placeholder="Evening Companion"
                    style={inputBase}
                  />
                </div>
                <div className="grid grid-cols-2 gap-3 mb-4">
                  <div>
                    ...
                    <select
                      value={card.sessionType}
                      onChange={(e) =>
                        setCards((p) =>
                          p.map((c) =>
                            c.id === card.id ? { ...c, sessionType: e.target.value } : c
                          )
                        )
                      }
                      style={{ ...inputBase, appearance: 'none' } as React.CSSProperties}
                  ...
                  <div>
                    ...
                    <select
                      value={card.durationKey}
                      onChange={(e) =>
                        setCards((p) =>
                          p.map((c) =>
                            c.id === card.id ? { ...c, durationKey: e.target.value } : c
                          )
                        )
                      }
                      style={{ ...inputBase, appearance: 'none' } as React.CSSProperties}
                ...
                <div className="grid grid-cols-2 gap-3 mb-4">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setCards((p) =>
                          p.map((c) => (c.id === card.id ? { ...c, price: e.target.value } : c))
                        )
                      }
                      placeholder="280"
                      style={inputBase}
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setCards((p) =>
                          p.map((c) =>
                            c.id === card.id
                              ? { ...c, currency: e.target.value as 'EUR' | 'USD' | 'GBP' }
                              : c
                          )
                        )
                      }
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      setCards((p) =>
                        p.map((c) => (c.id === card.id ? { ...c, description: e.target.value } : c))
                      )
                    }
            ...
            {cards.length < 5 && (
              <button
                type="button"
                onClick={() => setCards((p) => [...p, emptyCard()])}
                style={{
                  fontSize: 13,
                  color: '#e8607a',
            ...
            >
              ...
              {cards.filter((c) => c.title).length === 0 ? (
                ...
                cards
                  ...
                  .map((c) => {
                    const TypeIcon =
                      SESSION_TYPES.find((t) => t.value === c.sessionType)?.Icon ?? Users2
                    const durLabel =
                      DURATION_OPTIONS.find((d) => d.value === c.durationKey)?.label ??
                      c.durationKey
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span style={{ fontSize: 12, color: '#eeeef0', fontWeight: 500 }}>
                            {c.title}
                          </span>
                        </div>
                        <div className="flex items-center justify-between">
                          ...
                          {c.price && (
                            <span style={{ fontSize: 13, color: '#e8607a', fontWeight: 500 }}>
                              {sym}
                              {c.price}
                            </span>
                          )}
        ...
        <SaveButton
          ...
          onSave={() =>
            saveSection('D', {
              sessionCardsData: cards
                .filter((c) => c.title.trim() && c.price)
                .map((c) => ({
                  title: c.title,
      ...
      >
        <SectionHeader
          letter="E"
          title="Your Media"
          subtitle="Your visual presence. This is the first thing a dreamer sees."
        />
        ...
        <div className="mb-8">
          <p style={{ fontSize: 13, color: '#eeeef0', marginBottom: 6, fontWeight: 500 }}>
            ...
            <span style={{ color: '#6b7280', fontWeight: 400 }}>— min 3 required</span>
          </p>
          <div
          ...
          >
            <p style={{ fontSize: 13, color: '#6b7280' }}>
              Drop photos here or <span style={{ color: '#e8607a' }}>click to select</span>
            </p>
            <p style={{ fontSize: 11, color: '#6b7280', marginTop: 4 }}>
              JPG, PNG, WebP · Max 20 files
            </p>
          </div>
          <input
            ref={photoInputRef}
        ...
        <div>
          ...
          {!video ? (
            <>
              ...
              >
                <p style={{ fontSize: 13, color: '#6b7280' }}>
                  Drop video here or <span style={{ color: '#e8607a' }}>click to select</span>
                </p>
                <p style={{ fontSize: 11, color: '#6b7280', marginTop: 4 }}>
                  MP4 or MOV · Max 10 seconds · Max 50MB
                </p>
              </div>
              <input
                ref={videoInputRef}
              ...
              />
              {videoError && (
                <p style={{ fontSize: 11, color: '#e8607a', marginTop: 6 }}>{videoError}</p>
              )}
            </>
          ) : (
            <div
            ...
            >
              ...
              <div style={{ flex: 1 }}>
                {video.url ? (
                  <p style={{ fontSize: 12, color: '#eeeef0' }}>
                    Video ready · {video.durationSeconds}s
                  </p>
                ) : (
      ...
      >
        <SectionHeader
          ...
          subtitle="Verified companions get approved faster and appear with a badge."
        />
        ...
        {(verifyStatus === 'idle' || verifyStatus === 'declined') && (
          <div>
            {verifyStatus === 'declined' && (
              ...
              >
                <p style={{ fontSize: 12, color: '#e8607a' }}>
                  Your previous verification was declined. You can retry below.
                </p>
              </div>
            )}
            <button
      ...
      >
        <SectionHeader
          ...
          subtitle="Helps dreamers discover you. Everything here is optional but increases visibility."
        />

        <FieldWrap label="Gender identity">
          <div className="flex flex-wrap gap-2">
            {(['woman', 'man', 'non_binary', 'trans_woman', 'trans_man', 'other'] as const).map(
              (g) => {
                ...
                return (
                  <button
                    ...
                    style={{
                      borderColor: gender === g ? '#e8607a' : '#1c2333',
                      color: gender === g ? '#e8607a' : '#6b7280',
                      background: gender === g ? 'rgba(232,96,122,0.08)' : 'transparent',
                    }}
                  >
                    {label[g]}
                  </button>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <FieldWrap label="Body type">
            <div className="flex flex-wrap gap-2">
              {['slim', 'athletic', 'average', 'curvy', 'plus_size', 'prefer_not_to_say'].map(
                (b) => (
                  <button
                    key={b}
                    type="button"
                    ...
                    style={{
                      borderColor: bodyType === b ? '#e8607a' : '#1c2333',
                      color: bodyType === b ? '#e8607a' : '#6b7280',
                      background: bodyType === b ? 'rgba(232,96,122,0.08)' : 'transparent',
                    }}
                  >
                    {b.replace(/_/g, ' ')}
                  </button>
        ...
        <FieldWrap label="Ethnicity" helper="Free text — describe however feels right to you">
          <input
            ...
            style={{ ...inputBase }}
            placeholder="e.g. Mixed, Mediterranean, Nordic…"
          />
        </FieldWrap>
            ...
            <div className="flex flex-wrap gap-2">
              {['brown', 'blue', 'green', 'hazel', 'grey'].map((c) => (
                <button
                  ...
                  className="text-[12px] px-[12px] py-[11px] rounded-full border cursor-pointer transition-all duration-150 min-h-[44px]"
                  style={{
                    borderColor: eyeColor === c ? '#e8607a' : '#1c2333',
                    color: eyeColor === c ? '#e8607a' : '#6b7280',
                    background: eyeColor === c ? 'rgba(232,96,122,0.08)' : 'transparent',
                  }}
                >
                  {c}
                </button>
            ...
            <div className="flex flex-wrap gap-2">
              {['blonde', 'brown', 'black', 'auburn', 'grey', 'red', 'dark'].map((c) => (
                <button
                  ...
                  className="text-[12px] px-[12px] py-[11px] rounded-full border cursor-pointer transition-all duration-150 min-h-[44px]"
                  style={{
                    borderColor: hairColor === c ? '#e8607a' : '#1c2333',
                    color: hairColor === c ? '#e8607a' : '#6b7280',
                    background: hairColor === c ? 'rgba(232,96,122,0.08)' : 'transparent',
                  }}
                >
                  {c}
                </button>
          ...
          <div className="flex flex-wrap gap-2">
            {['fair', 'light', 'medium', 'olive', 'brown', 'dark'].map((c) => (
              <button
                ...
                className="text-[12px] px-[14px] py-[11px] rounded-full border cursor-pointer transition-all duration-150 min-h-[44px]"
                style={{
                  borderColor: skinColor === c ? '#e8607a' : '#1c2333',
                  color: skinColor === c ? '#e8607a' : '#6b7280',
                  background: skinColor === c ? 'rgba(232,96,122,0.08)' : 'transparent',
                }}
              >
                {c}
              </button>
      ...
      >
        <SectionHeader
          letter="H"
          title="Connect"
          subtitle="Private contact details used only for confirmed bookings."
        />
      ...
      >
        <SectionHeader
          letter="I"
          title="Availability & Go Live"
          subtitle="Control how dreamers can connect with you, and when you appear in the feed."
        />
            ...
            {(['in_person', 'online', 'both'] as const).map((m) => {
              ...
              return (
                <button
                  key={m}
                  type="button"
                  onClick={() => setSessionModality(m)}
                  className="flex-1 text-[12px] py-[13px] rounded-[10px] border cursor-pointer transition-all duration-150 font-medium min-h-[44px]"
                  style={{
                    borderColor: sessionModality === m ? '#e8607a' : '#1c2333',
                    color: sessionModality === m ? '#e8607a' : '#6b7280',
                    background: sessionModality === m ? 'rgba(232,96,122,0.08)' : 'transparent',
                  }}
                >
                  {labels[m]}
                </button>
        ...
        <div
          className="flex items-center justify-between p-4 rounded-[12px] mt-2"
          style={{ background: '#161d2a', border: '1px solid #1c2333' }}
        >
          <div>
            <p style={{ fontSize: 13, color: '#eeeef0', fontWeight: 500, marginBottom: 3 }}>
              Appear in the dreamer feed
            </p>
          ...
          <button
            ...
            style={{
              ...
              borderRadius: 12,
              border: 'none',
              cursor: profileCompleteness < 70 ? 'not-allowed' : 'pointer',
              background: isLive && profileCompleteness >= 70 ? '#e8607a' : '#1c2333',
              position: 'relative',
              transition: 'background 0.2s',
              opacity: profileCompleteness < 70 ? 0.5 : 1,
              flexShrink: 0,
            }}
          >
            <span
              style={{
                position: 'absolute',
                top: 2,
                left: isLive && profileCompleteness >= 70 ? 22 : 2,
                width: 20,
                height: 20,
                borderRadius: '50%',
                background: '#eeeef0',
        ...
        <SaveButton
          ...
          onSave={() =>
            saveSection('I', {
              ...
              is_live: profileCompleteness >= 70 ? isLive : undefined,
            })
          }
        />
      </motion.div>
