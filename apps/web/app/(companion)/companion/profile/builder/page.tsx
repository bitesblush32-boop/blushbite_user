...
import {
  ChevronDown, ChevronUp, Check, MapPin, User, Sparkles,
  Languages, Heart, Layers, Link, Shield, ToggleRight, AlertCircle,
} from 'lucide-react'
...
interface ProfileData {
  ...
  profile: {
    ...
    gender: string | null; skin_color: string | null
    session_modality: string; instagram_handle: string | null; website_url: string | null
    whatsapp_number: string | null; profile_completeness: number; is_visible_to_users: boolean
  }
  photos: { id: string; url: string; is_primary: boolean }[]
  verification: { provider_status: string; liveness_check_passed: boolean; verified_at: string | null } | null
  ...
  session_cards: { id: string; title: string; price: string | null; currency: string; duration_minutes: number | null; session_type: string | null; description: string | null }[]
}

type SectionId = 'identity' | 'location' | 'look' | 'languages' | 'vibe' | 'sessions' | 'connect' | 'verified'
  ...
  return (
    <div className="relative flex items-center justify-center" style={{ width: 120, height: 120 }}>
      <svg width={120} height={120} style={{ transform: 'rotate(-90deg)' }}>
        ...
        <motion.circle
          cx={60} cy={60} r={r} fill="none" stroke="#c9a96e" strokeWidth={6}
          strokeLinecap="round"
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <span style={{ fontSize: 26, fontWeight: 700, color: '#c9a96e', lineHeight: 1 }}>{pct}</span>
...
function SectionCard({
  id, icon, title, summary, isComplete, isExpanded, onToggle, children,
}: {
  id: SectionId; icon: React.ReactNode; title: string; summary: string
  isComplete: boolean; isExpanded: boolean; onToggle: () => void; children: React.ReactNode
}) {
  return (
    <div
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-2">
          {isComplete && (
            <span className="flex items-center justify-center w-[18px] h-[18px] rounded-full" style={{ background: 'rgba(201,169,110,0.18)' }}>
...
function FieldLabel({ children }: { children: React.ReactNode }) {
  return <div style={{ fontSize: 11, color: '#6b7280', marginBottom: 6, letterSpacing: '0.04em', textTransform: 'uppercase' }}>{children}</div>
}

function TextInput({
  value, onChange, onBlur, placeholder, multiline = false, maxLength,
}: {
  ...
  const base: React.CSSProperties = {
    ...
    color: '#eeeef0', outline: 'none', resize: 'none' as const,
  ...
  if (multiline) {
    return (
      <textarea
        ...
        style={base}
        onFocus={e => { (e.target as HTMLTextAreaElement).style.borderColor = 'rgba(232,96,122,0.5)' }}
      />
    )
  }
  return (
    <input
      type="text" value={value} onChange={e => onChange(e.target.value)} onBlur={onBlur}
      placeholder={placeholder} maxLength={maxLength} style={base}
      onFocus={e => { (e.target as HTMLInputElement).style.borderColor = 'rgba(232,96,122,0.5)' }}
    />
  )
}
...
function SaveIndicator({ saving }: { saving: boolean }) {
  return (
    <div style={{ fontSize: 11, color: saving ? '#c9a96e' : '#4b5563', marginTop: 8, height: 16, transition: 'color 0.3s' }}>
      {saving ? 'Saving…' : 'Saved'}
    </div>
  )
}
...
export default function ProfileBuilderPage() {
  ...
  const [fields, setFields] = useState({
    name: '', bio: '', tagline: '', city: '', availability_status: 'offline',
    session_modality: 'in_person', is_live: false,
    height_cm: '', body_type: '', ethnicity: '', eye_color: '', hair_color: '',
    gender: '', skin_color: '',
    instagram_handle: '', website_url: '', whatsapp_number: '',
    selected_vibe_tag_ids: [] as number[],
    selected_fantasy_tag_ids: [] as number[],
    selected_languages: [] as { language_id: number; fluency: string }[],
    session_cards: [] as { title: string; description: string; price: string; currency: string; duration_minutes: string; session_type: string }[],
  })
  ...
  const debouncedSave = useCallback((sectionId: SectionId, section: string, body: Record<string, unknown>, delay = 800) => {
  ...
  if (isLoading || !data) {
    return (
      <div className="flex flex-col gap-4 max-w-[640px] mx-auto px-5 pt-[95px] pb-[100px]">
        {[...Array(6)].map((_, i) => (
          <div key={i} className="h-[68px] rounded-[14px] animate-pulse" style={{ background: '#111620' }} />
        ))}
      </div>
    )
  }
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-center mb-8">
        ...
        <h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: 22, color: '#eeeef0', marginTop: 16, marginBottom: 4, textAlign: 'center' }}>
          Your world is{' '}
          <em style={{ fontStyle: 'italic', color: '#c9a96e' }}>{pct}% revealed</em>
        </h1>
        {pct < 100 && (
          <p style={{ fontSize: 12, color: '#6b7280', textAlign: 'center' }}>
            {Object.values(sectionComplete).filter(v => !v).length} section{Object.values(sectionComplete).filter(v => !v).length !== 1 ? 's' : ''} left to complete
          </p>
        )}
      </div>
      ...
      <div className="flex flex-col gap-3">
        {/* ── Identity ── */}
        <SectionCard
          id="identity" icon={<User size={16} />} title="Identity"
          summary={fields.name || 'Name, tagline, bio'}
          isComplete={sectionComplete.identity} isExpanded={expanded === 'identity'}
        ...
        >
          <div className="flex flex-col gap-4">
            <div>
              ...
              <TextInput
                value={fields.name} onChange={v => setFields(f => ({ ...f, name: v }))}
            ...
            <div>
              ...
              <TextInput
                value={fields.tagline} onChange={v => setFields(f => ({ ...f, tagline: v }))}
                placeholder="One line that captures your essence" maxLength={300}
            ...
            <div>
              ...
              <TextInput
                value={fields.bio} onChange={v => setFields(f => ({ ...f, bio: v }))}
                placeholder="Write as if you're whispering it…" multiline maxLength={2000}
        ...
        {/* ── Location ── */}
        <SectionCard
          id="location" icon={<MapPin size={16} />} title="Location & Availability"
          summary={fields.city || 'City, modality, go-live'}
          isComplete={sectionComplete.location} isExpanded={expanded === 'location'}
        ...
        >
          <div className="flex flex-col gap-4">
            <div>
              ...
              <TextInput
                value={fields.city} onChange={v => setFields(f => ({ ...f, city: v }))}
            ...
            <div>
              ...
              <div className="flex items-center justify-between" style={{ background: '#161d2a', borderRadius: 10, padding: '10px 14px', border: '1px solid #1c2333' }}>
                <div>
                  <div style={{ fontSize: 13, color: '#eeeef0' }}>Appear to dreamers</div>
                  <div style={{ fontSize: 11, color: '#6b7280', marginTop: 2 }}>
                    {isLiveGated ? `Complete ${70 - pct}% more to unlock` : 'Toggle your public visibility'}
                  </div>
                </div>
                <button
                  ...
                  style={{
                    width: 44, height: 24, borderRadius: 12, border: 'none', cursor: isLiveGated ? 'not-allowed' : 'pointer',
                    background: fields.is_live ? '#e8607a' : '#1c2333', position: 'relative', transition: 'background 0.2s',
                    opacity: isLiveGated ? 0.4 : 1,
                  }}
                >
                  <span style={{
                    position: 'absolute', top: 3, width: 18, height: 18, borderRadius: '50%', background: '#fff',
                    left: fields.is_live ? 23 : 3, transition: 'left 0.2s',
                  }} />
                </button>
              </div>
              {isLiveGated && (
                <div className="flex items-center gap-2 mt-2" style={{ fontSize: 11, color: '#6b7280' }}>
        ...
        {/* ── Your Look ── */}
        <SectionCard
          id="look" icon={<Sparkles size={16} />} title="Your Look"
          summary={[fields.gender, fields.body_type, fields.ethnicity].filter(Boolean).join(' · ') || 'Gender, body, features'}
          isComplete={sectionComplete.look} isExpanded={expanded === 'look'}
        ...
        >
          <div className="flex flex-col gap-4">
            <div>
              ...
              <PillSelect
                value={fields.gender}
                onChange={v => { setFields(f => ({ ...f, gender: v })); debouncedSave('look', 'physical', { gender: v || null }, 0) }}
                options={[
                  { value: 'woman', label: 'Woman' }, { value: 'man', label: 'Man' },
                  { value: 'non_binary', label: 'Non-binary' }, { value: 'trans_woman', label: 'Trans woman' },
                  { value: 'trans_man', label: 'Trans man' }, { value: 'other', label: 'Other' },
            ...
            <div>
              ...
              <TextInput
                value={fields.height_cm} onChange={v => setFields(f => ({ ...f, height_cm: v }))}
                placeholder="e.g. 168"
                onBlur={() => {
            ...
            <div>
              ...
              <PillSelect
                ...
                onChange={v => { setFields(f => ({ ...f, body_type: v })); debouncedSave('look', 'physical', { body_type: v || null }, 0) }}
                options={[
                  { value: 'slim', label: 'Slim' }, { value: 'athletic', label: 'Athletic' },
                  { value: 'average', label: 'Average' }, { value: 'curvy', label: 'Curvy' },
                  { value: 'plus_size', label: 'Plus size' }, { value: 'prefer_not_to_say', label: 'Prefer not' },
                ]}
              />
            </div>
            <div>
              ...
              <TextInput
                value={fields.ethnicity} onChange={v => setFields(f => ({ ...f, ethnicity: v }))}
                placeholder="e.g. Latina, East Asian, South Asian…" maxLength={50}
                onBlur={() => debouncedSave('look', 'physical', { ethnicity: fields.ethnicity || null })}
              />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <TextInput
                  value={fields.eye_color} onChange={v => setFields(f => ({ ...f, eye_color: v }))}
                  placeholder="Brown, hazel…" maxLength={30}
                  onBlur={() => debouncedSave('look', 'physical', { eye_color: fields.eye_color || null })}
                />
              </div>
              <div>
                <FieldLabel>Hair color</FieldLabel>
                <TextInput
                  value={fields.hair_color} onChange={v => setFields(f => ({ ...f, hair_color: v }))}
                  placeholder="Dark, auburn…" maxLength={30}
                  onBlur={() => debouncedSave('look', 'physical', { hair_color: fields.hair_color || null })}
                />
              </div>
            </div>
            <div>
              ...
              <PillSelect
                ...
                onChange={v => { setFields(f => ({ ...f, skin_color: v })); debouncedSave('look', 'physical', { skin_color: v || null }, 0) }}
                options={[
                  { value: 'fair', label: 'Fair' }, { value: 'light', label: 'Light' },
                  { value: 'medium', label: 'Medium' }, { value: 'olive', label: 'Olive' },
                  { value: 'brown', label: 'Brown' }, { value: 'dark', label: 'Dark' },
                  { value: 'ebony', label: 'Ebony' }, { value: 'prefer_not_to_say', label: 'Prefer not' },
                ]}
              />
            </div>
        ...
        {/* ── Languages ── */}
        <SectionCard
          id="languages" icon={<Languages size={16} />} title="Languages"
          summary={fields.selected_languages.length > 0
            ? fields.selected_languages.map(l => data.available_languages.find(a => a.id === l.language_id)?.name).filter(Boolean).join(', ')
            : 'Which languages do you speak?'}
          isComplete={sectionComplete.languages} isExpanded={expanded === 'languages'}
        ...
        {/* ── Your Vibe ── */}
        <SectionCard
          id="vibe" icon={<Heart size={16} />} title="Your Vibe"
          summary={fields.selected_vibe_tag_ids.length > 0
            ? `${fields.selected_vibe_tag_ids.length} vibe · ${fields.selected_fantasy_tag_ids.length} fantasy tags`
            : 'Vibe + fantasy tags'}
          isComplete={sectionComplete.vibe} isExpanded={expanded === 'vibe'}
        ...
        {/* ── Sessions ── */}
        <SectionCard
          id="sessions" icon={<Layers size={16} />} title="Sessions"
          summary={fields.session_cards.length > 0 ? `${fields.session_cards.length} session type${fields.session_cards.length > 1 ? 's' : ''}` : 'Define your offerings'}
          isComplete={sectionComplete.sessions} isExpanded={expanded === 'sessions'}
        ...
        {/* ── Connect ── */}
        <SectionCard
          id="connect" icon={<Link size={16} />} title="Connect"
          summary={fields.whatsapp_number || 'WhatsApp, Instagram, website'}
          isComplete={sectionComplete.connect} isExpanded={expanded === 'connect'}
        ...
        >
          <div className="flex flex-col gap-4">
            <div>
              ...
              <TextInput
                value={fields.whatsapp_number} onChange={v => setFields(f => ({ ...f, whatsapp_number: v }))}
                placeholder="+31612345678"
                onBlur={() => {
                  if (/^\+[1-9]\d{6,14}$/.test(fields.whatsapp_number) || fields.whatsapp_number === '')
                    debouncedSave('connect', 'whatsapp', { whatsapp_number: fields.whatsapp_number || null })
                }}
              />
            </div>
            <div>
              ...
              <TextInput
                value={fields.instagram_handle} onChange={v => setFields(f => ({ ...f, instagram_handle: v }))}
                placeholder="@yourhandle"
                onBlur={() => debouncedSave('connect', 'social', { instagram_handle: fields.instagram_handle || null })}
              />
            </div>
            <div>
              <FieldLabel>Website (optional)</FieldLabel>
              <TextInput
                value={fields.website_url} onChange={v => setFields(f => ({ ...f, website_url: v }))}
                placeholder="https://…"
                onBlur={() => debouncedSave('connect', 'social', { website_url: fields.website_url || null })}
              />
            </div>
        ...
        {/* ── Verified ── */}
        <SectionCard
          id="verified" icon={<Shield size={16} />} title="Verified"
          summary={data.verification?.provider_status === 'approved' ? '✦ Identity verified' : 'Complete ID verification'}
          isComplete={sectionComplete.verified} isExpanded={expanded === 'verified'}
          onToggle={() => toggleSection('verified')}
        >
          <VerificationDisplay verification={data.verification} onRetrigger={() => router.push('/companion/onboarding/verify')} />
        </SectionCard>
      </div>
...
}: { available: { id: number; name: string }[]; selected: number[]; onChange: (ids: number[]) => void }) {
  return (
    <div className="flex flex-wrap gap-2">
      {available.map(t => {
        ...
        return (
          <button
            key={t.id}
            onClick={() => onChange(active ? selected.filter(id => id !== t.id) : [...selected, t.id])}
            style={{
              fontSize: 11.5, padding: '5px 12px', borderRadius: 20, border: 'none', cursor: 'pointer',
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {selected.map(s => {
        ...
        if (!lang) return null
        return (
          <div key={s.language_id} className="flex items-center justify-between gap-2" style={{ background: '#161d2a', borderRadius: 10, padding: '8px 12px', border: '1px solid #1c2333' }}>
            ...
            <div className="flex items-center gap-1 flex-1 justify-center">
              {FLUENCY_OPTS.map(f => (
                <button
                  ...
                  style={{
                    fontSize: 10.5, padding: '3px 9px', borderRadius: 12, border: 'none', cursor: 'pointer',
                    background: s.fluency === f.value ? 'rgba(201,169,110,0.15)' : 'transparent',
                    color: s.fluency === f.value ? '#c9a96e' : '#6b7280',
                    outline: s.fluency === f.value ? '1px solid rgba(201,169,110,0.35)' : '1px solid transparent',
                    transition: 'all 0.15s',
                  }}
                >
                  {f.label}
            ...
            <button onClick={() => remove(s.language_id)} style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: '#4b5563', fontSize: 12, padding: 4 }}>✕</button>
          </div>
        )
      })}
      ...
      <input
        ...
        style={{
          width: '100%', background: '#161d2a', border: '1px solid #1c2333',
          borderRadius: 10, padding: '8px 12px', fontSize: 13, color: '#eeeef0',
          outline: 'none', fontFamily: 'inherit', boxSizing: 'border-box',
        }}
      />
      {search.length > 0 && filtered.length > 0 && (
        <div style={{ background: '#161d2a', border: '1px solid #1c2333', borderRadius: 10, overflow: 'hidden' }}>
          {filtered.map(l => (
            <button
              key={l.id}
              onClick={() => { add(l.id); setSearch('') }}
              style={{
                width: '100%', padding: '8px 14px', textAlign: 'left', background: 'transparent',
                border: 'none', cursor: 'pointer', fontSize: 13, color: '#eeeef0',
...
function SessionCardsEditor({
  ...
  cards: { title: string; description: string; price: string; currency: string; duration_minutes: string; session_type: string }[]
  onChange: (c: typeof cards) => void
}) {
  const add = () => onChange([...cards, { title: '', description: '', price: '', currency: 'EUR', duration_minutes: '', session_type: 'in_person' }])
  ...
  return (
    <div className="flex flex-col gap-3">
      {cards.map((c, i) => (
        <div key={i} style={{ background: '#161d2a', border: '1px solid #1c2333', borderRadius: 12, padding: '14px 14px 10px' }}>
          <div className="flex items-center justify-between mb-3">
            <span style={{ fontSize: 11, color: '#6b7280', textTransform: 'uppercase', letterSpacing: '0.04em' }}>Session {i + 1}</span>
            <button onClick={() => remove(i)} style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: '#4b5563', fontSize: 12 }}>Remove</button>
          </div>
          <div className="flex flex-col gap-2">
            <input
              placeholder="Session title" value={c.title}
              onChange={e => update(i, 'title', e.target.value)}
              style={{ background: '#111620', border: '1px solid #1c2333', borderRadius: 8, padding: '8px 10px', fontSize: 13, color: '#eeeef0', outline: 'none', fontFamily: 'inherit', width: '100%', boxSizing: 'border-box' }}
            />
            <div className="grid grid-cols-2 gap-2">
              <input
                placeholder="Price (e.g. 280)" value={c.price}
                onChange={e => update(i, 'price', e.target.value)}
                style={{ background: '#111620', border: '1px solid #1c2333', borderRadius: 8, padding: '8px 10px', fontSize: 13, color: '#eeeef0', outline: 'none', fontFamily: 'inherit', boxSizing: 'border-box' }}
              />
              <input
                placeholder="Duration (min)" value={c.duration_minutes}
      ...
      <button
        ...
        style={{
          ...
          fontSize: 13, color: '#6b7280', transition: 'border-color 0.15s, color 0.15s',
        }}
        onMouseEnter={e => { (e.target as HTMLElement).style.borderColor = 'rgba(232,96,122,0.4)'; (e.target as HTMLElement).style.color = '#e8607a' }}
        onMouseLeave={e => { (e.target as HTMLElement).style.borderColor = '#1c2333'; (e.target as HTMLElement).style.color = '#6b7280' }}
      >
        + Add session type
      </button>
    </div>
...
function VerificationDisplay({
  verification, onRetrigger,
}: { verification: ProfileData['verification']; onRetrigger: () => void }) {
  if (verification?.provider_status === 'approved') {
    return (
      <div className="flex items-center gap-3 p-4 rounded-[10px]" style={{ background: 'rgba(201,169,110,0.08)', border: '1px solid rgba(201,169,110,0.2)' }}>
        <Check size={18} color="#c9a96e" />
        <div>
          <div style={{ fontSize: 13, color: '#c9a96e', fontWeight: 500 }}>Identity verified</div>
          {verification.verified_at && (
            <div style={{ fontSize: 11, color: '#6b7280' }}>Verified {new Date(verification.verified_at).toLocaleDateString()}</div>
          )}
        </div>
      </div>
    )
      ...
      {!isPending && (
        <button
          ...
          style={{
            ...
            background: 'rgba(232,96,122,0.12)', color: '#e8607a', fontSize: 13,
            outline: '1px solid rgba(232,96,122,0.3)', transition: 'all 0.15s',
          }}
        >
          Start verification →
        </button>
