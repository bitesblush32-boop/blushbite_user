  ...
  return (
    ...
    >
      ...
      <div className="w-full max-w-[640px] px-5 md:px-6 py-8 flex flex-col items-center">
        <div className="w-full max-w-[580px] rounded-[16px] border border-[#1c2333]"
...
function StepPreview({
  ...
  useEffect(() => { setCurrentPage(0) }, [fontSize])
  ...
  return (
    ...
    >
      ...
      <div className="w-full max-w-[480px] mx-auto px-5 py-8 flex flex-col items-center">
        ...
        {selectedCategories.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-5 justify-center">
            {selectedCategories.slice(0, 4).map(cat => (
              <span key={cat} className="text-[11px] px-[10px] py-1 rounded-full text-[#e8607a]"
        ...
        <div className="relative w-full rounded-[16px] border border-[#1c2333] overflow-hidden"
          style={{ height: '52vh', background: gradient }}>
          <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', background: 'radial-gradient(ellipse 70% 35% at 50% 90%, rgba(232,96,122,0.10) 0%, transparent 70%)' }} />
          <StoryPageContent
            rawText={rawText}
        ...
        {/* Font size selector */}
        <div ref={fontMenuRef} style={{ position: 'relative', alignSelf: 'flex-start', marginTop: 14 }}>
          <button type="button" onClick={() => setFontMenuOpen(o => !o)}
            style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 11, color: '#6b7280', background: fontMenuOpen ? 'rgba(232,96,122,0.08)' : 'rgba(28,35,51,0.6)', border: `1px solid ${fontMenuOpen ? 'rgba(232,96,122,0.3)' : '#1c2333'}`, borderRadius: 999, padding: '4px 10px', cursor: 'pointer' }}>
            <Type size={11} />
          ...
          <AnimatePresence>
            {fontMenuOpen && (
              <motion.div
                ...
                style={{ position: 'absolute', top: 'calc(100% + 6px)', left: 0, background: '#161d2a', border: '1px solid #1c2333', borderRadius: 12, padding: '6px 4px', zIndex: 100, minWidth: 152, boxShadow: '0 8px 24px rgba(0,0,0,0.5)' }}
              >
                <p style={{ fontSize: 10, color: '#6b7280', padding: '2px 12px 6px', textTransform: 'uppercase', letterSpacing: '0.08em', margin: 0 }}>Text size</p>
                {(['sm', 'md', 'lg', 'xl'] as FontSize[]).map(s => (
                  <button key={s} type="button" onClick={() => { setFontSize(s); setFontMenuOpen(false) }}
                    style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%', padding: '7px 12px', fontSize: 13, color: fontSize === s ? '#e8607a' : '#eeeef0', background: fontSize === s ? 'rgba(232,96,122,0.08)' : 'none', border: 'none', cursor: 'pointer', borderRadius: 8 }}>
                    <span>{FONT_SIZE_LABELS[s]}</span>
                    <span style={{ fontSize: FONT_SIZE_PX[s], color: fontSize === s ? '#e8607a' : '#6b7280', lineHeight: 1 }}>Aa</span>
                  </button>
                ))}
              </motion.div>
            )}
        ...
        {/* Author */}
        <p style={{ fontSize: 13, color: '#6b7280', textAlign: 'center', marginTop: 12 }}>{alias}</p>
        ...
        <button type="button" onClick={onPublish} disabled={publishing}
          ...
          {publishing ? (
            <><Loader2 size={16} color="white" className="animate-spin" style={{ opacity: 0.8 }} />Sending into the dark…</>
          ) : (
      ...
      <TopBar onClose={() => {}} step={1} totalSteps={2} centerLabel="Preview" leftLabel="Back" onLeft={onBack} rightLabel="" onRight={() => {}} />
    </motion.div>
  )
}
