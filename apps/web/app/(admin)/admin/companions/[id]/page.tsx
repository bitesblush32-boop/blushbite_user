...
function OnboardingTracker({ progress }: { progress: CompanionDossier['onboarding_progress'] }) {
  ...
  progress.forEach(p => { byStage[p.stage] = p.status })
          ...
          <div key={s.n} className="flex items-center" style={{ flexShrink: 0 }}>
            <div className="flex flex-col items-center gap-1">
              <div
                ...
                style={{
                  background: isDone ? 'rgba(232,96,122,0.15)' : isRejected ? 'rgba(239,68,68,0.12)' : isActive ? 'rgba(232,96,122,0.08)' : '#111620',
                  border: isDone ? '1px solid rgba(232,96,122,0.5)' : isRejected ? '1px solid rgba(239,68,68,0.4)' : isActive ? '1px solid rgba(232,96,122,0.3)' : '1px solid #1c2333',
                  color: isDone ? '#e8607a' : isRejected ? '#ef4444' : isActive ? '#e8607a' : '#6b7280',
                }}
              >
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-[6px] z-[900] flex items-center justify-center px-4">
      ...
      >
        ...
        <div className="p-6">
          <h3 style={{ fontFamily: "'Playfair Display', serif" }} className="text-[20px] text-[#eeeef0] mb-2">{title}</h3>
...
export default function AdminCompanionDetailPage() {
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen p-6 md:p-10 flex items-center justify-center" style={{ background: '#07090f' }}>
    ...
    return (
      <div className="min-h-screen p-6 md:p-10 flex items-center justify-center" style={{ background: '#07090f' }}>
        <div className="text-center">
          ...
          <button onClick={() => router.back()} className="text-[#e8607a] text-[13px]">← Go back</button>
        </div>
      </div>
    )
  }

  const { companion, profile, verification, legal_docs, onboarding_progress, photos, videos, fantasy_tags, vibe_tags, session_cards } = data
    ...
    >
        ...
        <div className="bg-[#111620] border border-[#1c2333] rounded-[20px] p-6 mb-6">
          <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 style={{ fontFamily: "'Playfair Display', serif" }} className="text-[24px] text-[#eeeef0] mb-[2px]">
                  {companion.full_name ?? companion.alias ?? companion.email}
                </h1>
                <div className="text-[12px] text-[#6b7280]">{companion.email}</div>
                {companion.alias && <div className="text-[11px] text-[#6b7280]">{companion.alias}</div>}
              </div>
            </div>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {isPending && (
                <span className="text-[11px] px-[10px] py-1 rounded-full text-[#eeeef0]"
          ...
          <div className="mt-6 pt-5 border-t border-[#1c2333]">
            <div className="text-[11px] text-[#6b7280] uppercase tracking-[0.06em] mb-3">Onboarding Progress</div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="bg-[#111620] border border-[#1c2333] rounded-[14px] p-5">
              ...
              <dl className="flex flex-col gap-3">
                {[
                  ...
                  ['Date of birth', companion.date_of_birth ? new Date(companion.date_of_birth).toLocaleDateString('en-GB') : '—'],
                  ['Country', companion.country ?? '—'],
                  ['City', profile?.city ?? '—'],
                  ['Stage', companion.companion_stage != null ? `${companion.companion_stage}` : '—'],
                  ['Joined', new Date(companion.created_at).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })],
                ].map(([label, val]) => (
                  ...
                  <div className="flex flex-col gap-3">
                    {legal_docs.map(doc => (
                      <div key={doc.id} className="flex items-center justify-between">
                        <div>
                          <div className="text-[13px] text-[#eeeef0] capitalize">{doc.document_type.replace(/_/g, ' ')}</div>
                          {doc.signed_at && <div className="text-[11px] text-[#6b7280]">Signed {new Date(doc.signed_at).toLocaleDateString('en-GB')}</div>}
                          {doc.document_version && <div className="text-[10px] text-[#6b7280]">v{doc.document_version}</div>}
                        </div>
            ...
            <div className="bg-[#111620] border border-[#1c2333] rounded-[14px] p-5">
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="text-[13px] font-medium text-[#eeeef0]">Identity Verification</span>
              </div>
              {!verification
                ...
                : (
                  <dl className="flex flex-col gap-3">
                    {[
                      ...
                      ['Liveness', verification.liveness_check_passed === true ? '✓ Passed' : verification.liveness_check_passed === false ? '✗ Failed' : '—'],
                      ['Verified at', verification.verified_at ? new Date(verification.verified_at).toLocaleDateString('en-GB') : '—'],
                      ['Expires', verification.expires_at ? new Date(verification.expires_at).toLocaleDateString('en-GB') : '—'],
                    ].map(([label, val]) => (
                      <div key={label} className="flex items-start justify-between gap-3">
                        <dt className="text-[12px] text-[#6b7280] flex-shrink-0">{label}</dt>
                        <dd className="text-[13px] text-[#eeeef0] text-right">{val}</dd>
                    ...
                    {/* Doc thumbnails */}
                    {(verification.id_document_front_url || verification.id_document_back_url || verification.selfie_url) && (
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="bg-[#111620] border border-[#1c2333] rounded-[14px] p-5">
              ...
              {!profile
                ...
                : (
                  <div className="flex flex-col gap-3">
                    {profile.tagline && (
                      <p className="text-[13px] text-[#eeeef0] italic" style={{ fontFamily: "'Playfair Display', serif" }}>
                        "{profile.tagline}"
                      </p>
                    )}
                    {profile.bio && (
                    ...
                    <dl className="flex flex-col gap-2 mt-1">
                      {[
                        ['City', profile.city ?? '—'],
                        ['Rate', profile.hourly_rate ? `${profile.currency ?? ''} ${profile.hourly_rate}/hr` : '—'],
            ...
            {(fantasy_tags.length > 0 || vibe_tags.length > 0) && (
              <div className="bg-[#111620] border border-[#1c2333] rounded-[14px] p-5">
                ...
                {vibe_tags.length > 0 && (
                  <div className="mb-3">
                    <div className="text-[11px] text-[#6b7280] uppercase tracking-[0.05em] mb-2">Vibe</div>
                    <div className="flex flex-wrap gap-2">
                      {vibe_tags.map(t => (
                        <span key={t.id} className="text-[11px] px-[10px] py-1 rounded-full text-[#e8607a]"
                          style={{ background: 'rgba(232,96,122,0.08)', border: '1px solid rgba(232,96,122,0.25)' }}>
                          {t.emoji} {t.name}
                        </span>
                      ))}
                ...
                {fantasy_tags.length > 0 && (
                  <div>
                    <div className="text-[11px] text-[#6b7280] uppercase tracking-[0.05em] mb-2">Fantasy</div>
                    <div className="flex flex-wrap gap-2">
                      {fantasy_tags.map(t => (
                        <span key={t.id} className="text-[11px] px-[10px] py-1 rounded-full border border-[#1c2333] text-[#6b7280] bg-white/[0.03]">
                          {t.name}
                        </span>
                      ))}
                    </div>
            ...
            {session_cards.length > 0 && (
              <div className="bg-[#111620] border border-[#1c2333] rounded-[14px] p-5">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-[13px] font-medium text-[#eeeef0]">Sessions ({session_cards.length})</span>
                </div>
                <div className="flex flex-col gap-3">
                  {session_cards.map(sc => (
                    <div key={sc.id} className="bg-[#161d2a] border border-[#1c2333] rounded-[10px] px-4 py-3">
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          <div style={{ fontFamily: "'Playfair Display', serif" }} className="text-[14px] text-[#eeeef0]">{sc.title}</div>
                          {sc.description && <div className="text-[11.5px] text-[#6b7280] mt-1 leading-[1.5]">{sc.description}</div>}
                        </div>
                      ...
                      {sc.duration_minutes && (
                        <div className="text-[11px] text-[#6b7280] mt-2">{sc.duration_minutes} min · {sc.session_type ?? 'session'}</div>
                      )}
                    </div>
                  ))}
                </div>
        ...
        {photos.length > 0 && (
          <div className="bg-[#111620] border border-[#1c2333] rounded-[14px] p-5 mb-6">
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-[13px] font-medium text-[#eeeef0]">Photos ({photos.length})</span>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
              {photos.map(photo => (
                <div key={photo.id} className="relative group rounded-[10px] overflow-hidden bg-[#161d2a] border border-[#1c2333]" style={{ aspectRatio: '3/4' }}>
                  ...
                  <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-2 p-2">
                    {photo.is_primary && (
                      <span className="text-[10px] text-[#c9a96e] px-2 py-[2px] rounded-full"
                    ...
                    <div className="flex gap-2">
                      {!photo.is_approved && (
                        <button
                          onClick={() => photoMutation.mutate({ photo_id: photo.id, action: 'approve' })}
                      ...
                      <button
                        onClick={() => photoMutation.mutate({ photo_id: photo.id, action: 'reject' })}
                        className="w-8 h-8 rounded-full flex items-center justify-center text-white transition-all hover:scale-105"
                      ...
                      {!photo.is_primary && (
                        <button
                          onClick={() => photoMutation.mutate({ photo_id: photo.id, action: 'set_primary' })}
                          className="w-8 h-8 rounded-full flex items-center justify-center text-white transition-all hover:scale-105"
        ...
        {videos.length > 0 && (
          <div className="bg-[#111620] border border-[#1c2333] rounded-[14px] p-5 mb-6">
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-[13px] font-medium text-[#eeeef0]">Videos ({videos.length})</span>
            </div>
            <div className="flex flex-col gap-3">
              {videos.map(video => (
                <div key={video.id} className="bg-[#161d2a] border border-[#1c2333] rounded-[10px] px-4 py-3 flex items-center justify-between gap-4">
                  <div>
                    <div className="text-[13px] text-[#eeeef0]">Video</div>
                    {video.duration_seconds && (
                      <div className="text-[11px] text-[#6b7280] mt-[2px]">{video.duration_seconds}s</div>
                    )}
                  ...
                  <div className="flex items-center gap-2">
                    {video.is_approved
                      ? (
                        ...
                        <button
                          onClick={() => videoMutation.mutate({ video_id: video.id, action: 'approve' })}
      ...
      <div className="fixed bottom-0 left-0 right-0 md:left-[240px] z-[600] border-t border-[#1c2333] px-6 py-4 flex items-center justify-between gap-4"
        ...
        <div className="flex gap-3 ml-auto">
          ...
          {isLive && (
            <button
              onClick={() => photoMutation.mutate({ photo_id: '', action: '' }) /* no-op placeholder */}
