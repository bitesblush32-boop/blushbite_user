...
function eventLabel(e: ActivityEvent): string {
  switch (e.type) {
    case 'new_dreamer':        return e.alias ? `${e.alias} joined` : 'New dreamer joined'
    case 'new_companion':      return `New companion applied`
    case 'companion_approved': return 'Companion went live'
    case 'story_submitted':    return e.title ? `"${e.title}" submitted` : 'Story submitted'
    case 'story_approved':     return e.title ? `"${e.title}" approved` : 'Story approved'
    ...
    >
      ...
      {isLoading ? (
        <div className="flex flex-col gap-3">
          {[0,1,2,3,4,5].map(i => (
            <div key={i} className="bg-[#111620] border border-[#1c2333] rounded-[14px] h-[64px] animate-pulse" />
          ))}
        </div>
      ) : filtered.length === 0 ? (
        <p
          style={{ fontFamily: "'Playfair Display', serif", fontSize: 18, color: '#6b7280', fontStyle: 'italic', textAlign: 'center', padding: '60px 0' }}
        >
          No events yet.
        </p>
      ) : (
