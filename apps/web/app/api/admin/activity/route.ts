...
import { db } from '@/db'
import { users, companions, companionProfiles, stories, bookingRequests } from '@/db/schema'
...
export async function GET(req: NextRequest) {
  ...
  const [
    ...
    db
      .select({ id: users.id, alias: users.alias, created_at: users.created_at })
      .from(users)
    ...
    db
      .select({ id: stories.id, title: stories.title, moderated_at: stories.moderated_at })
      .from(stories)
      .where(
        and(
          eq(stories.moderation_status, 'approved'),
          isNotNull(stories.moderated_at),
          isNull(stories.deleted_at)
        )
      )
    ...
    db
      .select({ id: bookingRequests.id, created_at: bookingRequests.created_at })
      .from(bookingRequests)
      .orderBy(desc(bookingRequests.created_at))
      .limit(10),

    db
      .select({ id: bookingRequests.id, updated_at: bookingRequests.updated_at })
      .from(bookingRequests)
