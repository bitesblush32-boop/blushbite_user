...
export async function GET(_req: NextRequest) {
  ...
  const [
    ...
    }).from(fantasyCategories).orderBy(asc(fantasyCategories.sort_order)),
    ...
    }).from(vibeTags).orderBy(asc(vibeTags.id)),
    ...
    }).from(moodTags).orderBy(asc(moodTags.id)),
    ...
    }).from(orientationTags).orderBy(asc(orientationTags.id)),
    ...
    }).from(storyCategories).orderBy(asc(storyCategories.sort_order)),
  ])
...
export async function POST(req: NextRequest) {
  ...
  if (table === 'fantasy_category') {
    const [r] = await db.insert(fantasyCategories).values({
    ...
    if (!category_id) return NextResponse.json({ error: 'category_id required for fantasy_tag' }, { status: 400 })
    const [r] = await db.insert(fantasyTags).values({
      name: name.trim(), slug: finalSlug,
      category_id, description: description ?? null,
    }).returning({ id: fantasyTags.id })
    id = r.id
  } else if (table === 'vibe_tag') {
    const [r] = await db.insert(vibeTags).values({
      name: name.trim(), slug: finalSlug, emoji: emoji ?? null,
    }).returning({ id: vibeTags.id })
    id = r.id
  } else if (table === 'mood_tag') {
    const [r] = await db.insert(moodTags).values({
      name: name.trim(), slug: finalSlug, emoji: emoji ?? null,
    }).returning({ id: moodTags.id })
    id = r.id
  } else if (table === 'orientation_tag') {
    const [r] = await db.insert(orientationTags).values({
      name: name.trim(), slug: finalSlug,
    }).returning({ id: orientationTags.id })
    id = r.id
  } else if (table === 'story_category') {
    const [r] = await db.insert(storyCategories).values({
      name: name.trim(), slug: finalSlug,
      description: description ?? null,
      sort_order: sort_order ?? 0,
