...
import {
  ...
  storyCategories, moodTags, orientationTags, fantasyTags,
} from '@/db/schema'
...
export async function POST(req: NextRequest) {
  ...
  const [cats, moods, orientations, fantasy] = await Promise.all([
    db.select({ id: storyCategories.id, name: storyCategories.name, slug: storyCategories.slug }).from(storyCategories),
  ...
  for (const item of items) {
    ...
    if (!title || !bodyText) { skipped++; continue }
    ...
    for (const cn of catNames) {
      ...
      if (found) { category_id = found; break }
    }

    const moodIds    = (item.mood_tags ?? []).map(n => moodByName.get(n.toLowerCase())).filter((v): v is number => v != null)
    const orientIds  = (item.orientation_tags ?? []).map(n => orientByName.get(n.toLowerCase())).filter((v): v is number => v != null)
    const fantasyIds = (item.fantasy_tags ?? []).map(n => fantasyByName.get(n.toLowerCase())).filter((v): v is number => v != null)
    ...
    await Promise.all([
      moodIds.length    ? db.insert(storyMoodTags).values(moodIds.map(tid => ({ story_id: storyId, mood_tag_id: tid })))           : Promise.resolve(),
      orientIds.length  ? db.insert(storyOrientationTags).values(orientIds.map(tid => ({ story_id: storyId, orientation_tag_id: tid }))) : Promise.resolve(),
      fantasyIds.length ? db.insert(storyFantasyTags).values(fantasyIds.map(tid => ({ story_id: storyId, fantasy_tag_id: tid })))   : Promise.resolve(),
    ])
