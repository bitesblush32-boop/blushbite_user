...
export async function POST(req: NextRequest) {
  ...
  await Promise.all([
    (mood_tag_ids as number[]).length
      ? db.insert(storyMoodTags).values((mood_tag_ids as number[]).map(tid => ({ story_id: storyId, mood_tag_id: tid })))
      : Promise.resolve(),
    (orientation_tag_ids as number[]).length
      ? db.insert(storyOrientationTags).values((orientation_tag_ids as number[]).map(tid => ({ story_id: storyId, orientation_tag_id: tid })))
      : Promise.resolve(),
    (fantasy_tag_ids as number[]).length
      ? db.insert(storyFantasyTags).values((fantasy_tag_ids as number[]).map(tid => ({ story_id: storyId, fantasy_tag_id: tid })))
      : Promise.resolve(),
  ])
