...
import { db } from '@/db'
import { companionStoryBridges, companionProfiles, companions, companionPhotos } from '@/db/schema'
...
export async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const rows = await db
      ...
      .innerJoin(
        companionProfiles,
        eq(companionProfiles.id, companionStoryBridges.companion_profile_id)
      )
