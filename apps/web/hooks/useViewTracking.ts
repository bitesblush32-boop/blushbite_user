...
export function useViewTracking(minViewDuration = 2000) {
  ...
  const observe = useCallback((storyId: string, el: HTMLElement | null) => {
    // Disconnect any previous observer for this id
    const existing = observers.current.get(storyId)
    if (existing) { existing.disconnect(); observers.current.delete(storyId) }
    if (!el) return
      ...
      (entries) => {
        for (const entry of entries) {
          if (entry.isIntersecting) {
            ...
            if (timer) { clearTimeout(timer); timers.current.delete(storyId) }
          }
        }
      },
      { threshold: 0.5 }
