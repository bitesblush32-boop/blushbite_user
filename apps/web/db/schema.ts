...
export const fantasyTags = pgTable('fantasy_tags', {
  id:          serial('id').primaryKey(),
  category_id: integer('category_id').notNull().references(() => fantasyCategories.id),
  name:        varchar('name', { length: 100 }).notNull(),
  slug:        varchar('slug', { length: 100 }).notNull().unique(),
  description: text('description'),
  is_active:   boolean('is_active').notNull().default(true),
...
export const userAccounts = pgTable('user_accounts', {
  ...
  providerUnique: unique('idx_user_accounts_provider').on(table.provider, table.provider_account_id),
}))
...
export const userFantasyTags = pgTable('user_fantasy_tags', {
  user_id:        uuid('user_id').notNull().references(() => users.id, { onDelete: 'cascade' }),
...
export const companionAccounts = pgTable('companion_accounts', {
  id:                  uuid('id').primaryKey().defaultRandom(),
  companion_id:        uuid('companion_id').notNull().references(() => companions.id, { onDelete: 'cascade' }),
  provider:            varchar('provider', { length: 255 }).notNull(),
  provider_account_id: varchar('provider_account_id', { length: 255 }).notNull(),
  type:                varchar('type', { length: 255 }).notNull(),
  refresh_token:       text('refresh_token'),
  ...
  providerUnique: unique('idx_companion_accounts_provider').on(table.provider, table.provider_account_id),
}))
...
export const companionPhotos = pgTable('companion_photos', {
  id:                   uuid('id').primaryKey().defaultRandom(),
  companion_profile_id: uuid('companion_profile_id').notNull().references(() => companionProfiles.id, { onDelete: 'cascade' }),
...
export const companionVideos = pgTable('companion_videos', {
  id:                   uuid('id').primaryKey().defaultRandom(),
  companion_profile_id: uuid('companion_profile_id').notNull().references(() => companionProfiles.id, { onDelete: 'cascade' }),
  url:                  text('url').notNull(),
  storage_key:          text('storage_key').notNull(),
...
export const companionLanguages = pgTable('companion_languages', {
  companion_profile_id: uuid('companion_profile_id').notNull().references(() => companionProfiles.id, { onDelete: 'cascade' }),
  ...
  fluencyCheck: check('cl_fluency_check', sql`${table.fluency} IN ('native', 'fluent', 'conversational')`),
}))

export const companionVibeTags = pgTable('companion_vibe_tags', {
  companion_profile_id: uuid('companion_profile_id').notNull().references(() => companionProfiles.id, { onDelete: 'cascade' }),
  vibe_tag_id:          integer('vibe_tag_id').notNull().references(() => vibeTags.id),
}, (table) => ({
  pk: primaryKey({ columns: [table.companion_profile_id, table.vibe_tag_id] }),
}))

export const companionFantasyTags = pgTable('companion_fantasy_tags', {
  companion_profile_id: uuid('companion_profile_id').notNull().references(() => companionProfiles.id, { onDelete: 'cascade' }),
  fantasy_tag_id:       integer('fantasy_tag_id').notNull().references(() => fantasyTags.id),
}, (table) => ({
...
export const sessionCards = pgTable('session_cards', {
  ...
  sessionTypeCheck: check('sc_session_type_check', sql`${table.session_type} IN ('in_person', 'audio_call', 'video_call', 'chat', 'custom')`),
}))

export const companionOnboardingProgress = pgTable('companion_onboarding_progress', {
  id:           uuid('id').primaryKey().defaultRandom(),
  companion_id: uuid('companion_id').notNull().references(() => companions.id, { onDelete: 'cascade' }),
  ...
  statusCheck:          check('cop_status_check', sql`${table.status} IN ('completed', 'rejected', 'skipped')`),
}))

export const companionVerifications = pgTable('companion_verifications', {
  id:                    uuid('id').primaryKey().defaultRandom(),
  companion_id:          uuid('companion_id').notNull().unique().references(() => companions.id),
...
export const companionLegalDocs = pgTable('companion_legal_docs', {
  id:               uuid('id').primaryKey().defaultRandom(),
  companion_id:     uuid('companion_id').notNull().references(() => companions.id),
  ...
  uniqueCompanionDoc: unique('uq_companion_legal_doc').on(table.companion_id, table.document_type),
  docTypeCheck:       check('cld_doc_type_check', sql`${table.document_type} IN ('tos', 'model_release', 'form_2257')`),
}))
...
export const stories = pgTable('stories', {
  ...
  authorTypeCheck:  check('st_author_type_check', sql`${table.author_type} IN ('admin', 'companion', 'user')`),
  moderationCheck:  check('st_moderation_check', sql`${table.moderation_status} IN ('pending', 'approved', 'rejected')`),
}))

export const storyMoodTags = pgTable('story_mood_tags', {
  story_id:    uuid('story_id').notNull().references(() => stories.id, { onDelete: 'cascade' }),
  mood_tag_id: integer('mood_tag_id').notNull().references(() => moodTags.id),
}, (table) => ({
  pk: primaryKey({ columns: [table.story_id, table.mood_tag_id] }),
}))

export const storyOrientationTags = pgTable('story_orientation_tags', {
  story_id:           uuid('story_id').notNull().references(() => stories.id, { onDelete: 'cascade' }),
  orientation_tag_id: integer('orientation_tag_id').notNull().references(() => orientationTags.id),
}, (table) => ({
  pk: primaryKey({ columns: [table.story_id, table.orientation_tag_id] }),
}))

export const storyFantasyTags = pgTable('story_fantasy_tags', {
  story_id:       uuid('story_id').notNull().references(() => stories.id, { onDelete: 'cascade' }),
  fantasy_tag_id: integer('fantasy_tag_id').notNull().references(() => fantasyTags.id),
}, (table) => ({
...
export const audioRecordings = pgTable('audio_recordings', {
  ...
  authorTypeCheck: check('ar_author_type_check', sql`${table.author_type} IN ('companion', 'user')`),
  moderationCheck: check('ar_moderation_check', sql`${table.moderation_status} IN ('pending', 'approved', 'rejected')`),
}))
...
export const likes = pgTable('likes', {
  ...
  contentTypeCheck: check('lk_content_type_check', sql`${table.content_type} IN ('story', 'audio_recording', 'comment', 'companion_profile')`),
}))
...
export const comments = pgTable('comments', {
  ...
  contentTypeCheck: check('cm_content_type_check', sql`${table.content_type} IN ('story', 'audio_recording')`),
  moderationCheck:  check('cm_moderation_check', sql`${table.moderation_status} IN ('pending', 'approved', 'rejected')`),
}))
...
export const bookingRequests = pgTable('booking_requests', {
  ...
  companion_profile_id:       uuid('companion_profile_id').notNull().references(() => companionProfiles.id),
